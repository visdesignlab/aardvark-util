...
def run_gui():
    ...
    # Input section
    tk.Label(root, text="Convert Trackmate Data to Loon Data", font=("Arial", 16)).pack(pady=10)
    ...
    tk.Entry(frame, textvariable=csv_var, width=40).grid(row=0, column=1)
    tk.Button(frame, text="Browse", command=lambda: csv_var.set(filedialog.askopenfilename(title="Select TrackMate CSV File", filetypes=[("CSV Files", "*.csv"), ("All Files", "*.*")]))).grid(row=0, column=2)
    tk.Label(frame, text="ROI Folder:").grid(row=1, column=0, sticky="e")
    tk.Entry(frame, textvariable=roi_var, width=40).grid(row=1, column=1)
    tk.Button(frame, text="Browse", command=lambda: roi_var.set(filedialog.askdirectory(title="Select ROI Folder"))).grid(row=1, column=2)
    tk.Label(frame, text="Output Folder:").grid(row=2, column=0, sticky="e")
    tk.Entry(frame, textvariable=output_var, width=40).grid(row=2, column=1)
    tk.Button(frame, text="Browse", command=lambda: output_var.set(filedialog.askdirectory(title="Select Output Folder"))).grid(row=2, column=2)
    # Output section (hidden until Convert is clicked)
    output_frame = tk.Frame(root)
    text_box = tk.Text(output_frame, wrap=tk.NONE, height=12, state=tk.DISABLED)
    ...
    tk.Button(root, text="Convert", command=run_conversion, font=("Arial", 12)).pack(pady=20)
    root.mainloop()
